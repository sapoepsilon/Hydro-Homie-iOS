...
//   @State private var bullteIn = BLTNItemManager()
    ...
    @State private var бюллетень: Bool = true
...
//            } else {
                if user.loggedIn {
                    if user.isUploadFinished {
                        Dashboard(isDocumentAddition: $registerView, isLoad: $isLoad, customDrinkDocument: CustomDrinkViewModel(), backgroundColorTop: $backgroundColorTop, backgroundColorBottom: $backgroundColorBottom)
...
//
        ...
        .addBoard(board: {OnboardScreen( waterColor: $waterColor)}, isShowing: $isWelcomePageShown)
        .onAppear{
            if colorScheme == .dark {
        ...
        .sheet(item: $activeSheet, onDismiss: {
            ...
            switch sheet {
            case .isRegister:
                RegisterView(isUserExist: $isUserExist, onCompleteBlock: {activeSheet = nil}, isLoad: $isLoad, Dashboard: $signIn, registerView: self.$registerView)
                    .environmentObject(user)
