...
//      @State var selectedEntry: ChartDataEntry?
    ...
    var body: some View {
        ...
        ZStack {
            ...
            VStack {
                VStack(spacing: 10) {
                    ...
                    HStack {
                        ...
                        if user.getHydrationArrayFromTheUserDefaults().count >= 7 {
                            ForEach(user.getHydrationArrayFromTheUserDefaults().suffix(7), id: \.self) { dictionary in
                        ...
                        } else if user.getHydrationArrayFromTheUserDefaults().count < 7 {
                            ForEach(user.getHydrationArrayFromTheUserDefaults(), id: \.self) { dictionary in
                                weeklyRings(hydration: dictionary)
                                        .shadow(color: .black, radius: 2, x: 0.0, y: /*@START_MENU_TOKEN@*/0.0/*@END_MENU_TOKEN@*/)
                                        .frame(width: UIScreen.main.bounds.width / 8.4).padding(.top)
                            }
            ...
            if weeklyOnAppear == 0 {
                ...
                print("arrayOfwater \(arrayOfWater)")
            }
    ...
    func weeklyRings(hydration: [String: [String:Double]]) -> some View {
        print("Value of the hydration \(hydration.description)")



            var waterPercentage: Double = getTheDrinkValue(hydrationDictionary: hydration, drinkName: "water")
            var alcoholPercentage: Double = getTheDrinkValue(hydrationDictionary: hydration, drinkName: "alcohol")
            var coffeePercentage: Double = getTheDrinkValue(hydrationDictionary: hydration, drinkName: "coffee")
        print("water Percentage: \(waterPercentage) coffee percentage: \(coffeePercentage) alcohol Percentage: \(alcoholPercentage)")
    ...
    func getOCKData()  {
        for hydration in user.getHydrationArrayFromTheUserDefaults() {
            for drinks in  hydration.values {
                print("hydration keys \(hydration.keys)")
                print("today's date in the userDefault \(UserDefaults.standard.string(forKey: "today"))")
                if hydration.keys.debugDescription != UserDefaults.standard.string(forKey: "today") {
                    arrayOfWater.append(CGFloat(drinks["water"] ?? 0))
                }
            }
        }
    }
